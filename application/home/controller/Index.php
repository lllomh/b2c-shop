 ...
 * xms
 * ============================================================================
 * * 版权所有 2015-2027 雪漫科技有限公司，并保留所有权利。
 * 网站地址: http://www.xms.cn
 * ----------------------------------------------------------------------------
 * .
 * ============================================================================
 * $ 2015-08-10 $
...
class Index extends Base {
    ...
    public function index(){      
        ...
        if(empty($index_hot_goods))
        {
            ...
            S('index_hot_goods',$index_hot_goods,xms_CACHE_TIME);
        }
        ...
        $hot_category = M('goods_category')->where("is_hot=1 and level=3 and is_show=1")->cache(true,xms_CACHE_TIME)->select();//热门三级分类
    ...
    public function qr_code(){        
        ...
        //#Home/Index/erweima/data/www.99xmshop.com
    ...
    function truncate_tables (){
        ...
        {                                    
            if(!in_array($val['Tables_in_xms2.0'], $table))                             
                echo "truncate table ".$val['Tables_in_xms2.0'].' ; ';
                echo "<br/>";         
        }                
    }
    ...
    public function ajax_favorite(){
        $p = I('p/d',1);
        $i = I('i',5); //显示条数
        $favourite_goods = M('goods')->where("is_recommend=1 and is_on_sale=1")->order('goods_id DESC')->page($p,$i)->cache(true,xms_CACHE_TIME)->select();//首页推荐商品
