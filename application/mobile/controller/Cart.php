...
class Cart extends MobileBase {
    ...
    public function cart(){
        //获取热卖商品
        $hot_goods = M('Goods')->where('is_hot=1 and is_on_sale=1')->limit(20)->cache(true,xms_CACHE_TIME)->select();
...
//        $result = $this->cartLogic->cartList($this->user, $this->session_id,1); // 获取购物车商品
        $shippingList = M('Plugin')->where("`type` = 'shipping' and status = 1")->cache(true,xms_CACHE_TIME)->select();// 物流公司
