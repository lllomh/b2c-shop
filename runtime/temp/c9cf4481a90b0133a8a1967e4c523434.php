<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:39:"./template/mobile/new2/index\index.html";i:1511762917;s:41:"./template/mobile/new2/public\footer.html";i:1511846782;s:45:"./template/mobile/new2/public\footer_nav.html";i:1495621986;s:43:"./template/mobile/new2/public\wx_share.html";i:1511762916;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8" />
...
<body>
    ...
    <div class="banner ban1">
        <div class="mslide" id="slidexms">
            <ul>
                <!--广告表-->
                <?php $pid =2;$ad_position = M("ad_position")->cache(true,xms_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1511935200 and end_time > 1511935200 ")->order("orderby desc")->cache(true,xms_CACHE_TIME)->limit("5")->select();
...
foreach($result as $key=>$v):       
    ...
    <div class="banner ma-to-20">
        <?php $pid =400;$ad_position = M("ad_position")->cache(true,xms_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1511935200 and end_time > 1511935200 ")->order("orderby desc")->cache(true,xms_CACHE_TIME)->limit("1")->select();
if(!in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
    ...
    <div class="floor advertisement">
        <div class="content">
            <div class="le lefhe fl">
                <?php $pid =301;$ad_position = M("ad_position")->cache(true,xms_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1511935200 and end_time > 1511935200 ")->order("orderby desc")->cache(true,xms_CACHE_TIME)->limit("1")->select();
if(!in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
...
foreach($result as $key=>$v):       
    ...
    ?>
                ...
                <?php endforeach; $pid =302;$ad_position = M("ad_position")->cache(true,xms_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1511935200 and end_time > 1511935200 ")->order("orderby desc")->cache(true,xms_CACHE_TIME)->limit("1")->select();
if(!in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
...
foreach($result as $key=>$v):       
    ...
    ?>
            ...
            <div class="le re fr">
                <?php $pid =300;$ad_position = M("ad_position")->cache(true,xms_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1511935200 and end_time > 1511935200 ")->order("orderby desc")->cache(true,xms_CACHE_TIME)->limit("1")->select();
if(!in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
...
foreach($result as $key=>$v):       
        ...
        <div class="hotsome">
            <div class="bloc hottop">
                <?php
                                ...
                                $md5_key = md5("select * from __PREFIX__goods where is_hot=1 and is_on_sale = 1 order by sort limit 2");
                                $result_name = $sql_result_v = S("sql_".$md5_key);
                                if(empty($sql_result_v))
                                {                            
                                    $result_name = $sql_result_v = \think\Db::query("select * from __PREFIX__goods where is_hot=1 and is_on_sale = 1 order by sort limit 2"); 
                                    S("sql_".$md5_key,$sql_result_v,1);
                                }    
                              foreach($sql_result_v as $k=>$v): ?>
                    <div class="le fl">
                        <a href="<?php echo U('Mobile/Goods/goodsInfo',array('id'=>$v[goods_id])); ?>"><img src="<?php echo goods_thum_images($v['goods_id'],400,400); ?>"/></a>
                    </div>
                    <!--<div class="le fr">-->
                        <!--<a href=""><img src="__STATIC__/images/ind_38.jpg"/></a>-->
                    <!--</div>-->
                <?php endforeach; ?>
            </div>
            <div class="bloc">
                <div class="le foura">
                    <?php
                              ...
                              foreach($sql_result_v as $k=>$v): ?>
                    <!--<a href=""><img src="__STATIC__/images/ind_42.jpg"/></a>-->
                    <a href="<?php echo U('Mobile/Goods/goodsInfo',array('id'=>$v[goods_id])); ?>"><img src="<?php echo goods_thum_images($v['goods_id'],400,600); ?>"/></a>
                    <?php endforeach; ?>
                </div>
            </div>
        </div>
    ...
    <div class="floor guesslike">
        ...
        <!--<div class="loadbefore" style="display:none;">-->
    ...
    <div class="flool1">
        <ul>
            ...
            <?php endif; ?>
            <li><a href="tel:<?php echo $xms_config['shop_info_phone']; ?>">反馈</a></li>
    ...
    <div class="flool3">
        <p>Copyright © 2004-2016 xms雪漫99soubao.com版权所有</p>
    </div>
</footer>
    <!--底部-end-->
...
<?php else: ?>
   ...
   var ShareImgUrl = "http://<?php echo $_SERVER[HTTP_HOST]; ?><?php echo $xms_config['shop_info_store_logo']; ?>"; //分享图标
<?php endif; ?>
	...
	if(isWeiXin() && parseInt(user_id)>0){
		...
		wx.ready(function(){
		    ...
		    wx.onMenuShareTimeline({
		        title: "<?php echo $xms_config['shop_info_store_title']; ?>", // 分享标题
		        link:ShareLink,
		    ...
		    wx.onMenuShareAppMessage({
		        title: "<?php echo $xms_config['shop_info_store_title']; ?>", // 分享标题
		        desc: "<?php echo $xms_config['shop_info_store_desc']; ?>", // 分享描述
		        link:ShareLink,
		        imgUrl:ShareImgUrl // 分享图标
		    });
			// 分享到QQ
