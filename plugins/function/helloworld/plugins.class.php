...
{    
    ...
    function install()
    {                                     
        $xms_version = file_get_contents($this->app_path.'/Application/Admin/Conf/version.txt'); // xms 版本
        $config = include $this->app_path.'plugins/function/helloworld/config.php'; // 当前插件适合哪些版本
        $config['version'] = explode(',', $config['version']);
        if(!in_array($xms_version, $config['version']))
        {
            $info['status'] = 0;
            $info['msg'] = '版本不兼容';
            return $info;                   
        ...
        // 而 MD5 对比值 则预先在 xms 原始文件中, 通过 MD5_file 获取到  放在这里
